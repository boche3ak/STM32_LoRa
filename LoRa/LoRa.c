...
LoRa newLoRa(){
  ...
  new_LoRa.crcRate               = CR_4_8    ; //we consider working in high-interference conditions, let's try the highest. original CR_4_5
...
uint8_t LoRa_isvalid(LoRa* _LoRa){
  ...
  if(_LoRa->CS_port == NULL || _LoRa->reset_port == NULL || _LoRa->DIO0_port == NULL)
    return 0;
  if(_LoRa->CS_pin == 0 || _LoRa->reset_pin == 0 || _LoRa->DIO0_pin == 0)
    return 0;
  if(_LoRa->hSPIx == NULL)
    return 0;
  return 1;
